...
export const generateSitemap = asyncHandler(async (req, res) => {
  try {
    ...
    // Convert database data to sitemap entries with proper URL encoding
    const dynamicRoutes = [
      ...snippets.map(snippet => ({
        url: `/snippets/${snippet._id.toString()}`.replace(/[<>&'"\s]/g, function(c) {
      ...
      ...collections.map(collection => ({
        url: `/collections/${collection._id.toString()}`.replace(/[<>&'"\s]/g, function(c) {
          return '&#' + c.charCodeAt(0) + ';';
        }),
      ...
      ...users.map(user => ({
        url: `/user/${user.username}`.replace(/[<>&'"\s]/g, function(c) {
          return '&#' + c.charCodeAt(0) + ';';
        }),
